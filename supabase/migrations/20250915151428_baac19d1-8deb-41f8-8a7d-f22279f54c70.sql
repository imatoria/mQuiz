-- Phase 2: Database Functions & RLS Migration (Safe Migration)
...
-- Create trigger for paper attempt activity updates (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'paper_attempts') THEN
...
-- Create audit triggers for paper-related tables (only if tables exist)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'paper_attempts') THEN
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'paper_assignments') THEN
...
-- Create performance indexes (only if tables exist)
DO $$
BEGIN
  -- Indexes for question_papers
  ...
  -- Indexes for paper tables (if they exist)
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'paper_attempts') THEN
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'paper_sessions') THEN
    CREATE INDEX IF NOT EXISTS idx_paper_sessions_attempt_active ON paper_sessions(paper_attempt_id, is_active);
  END IF;
  
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'paper_violations') THEN
