...
create policy "Users can insert their own page selections"
  on public.document_page_selections for insert
  with check (auth.uid() = user_id);

create policy "Users can view their own page selections"
  on public.document_page_selections for select
  using (auth.uid() = user_id);

create policy "Users can update their own page selections"
  on public.document_page_selections for update
  using (auth.uid() = user_id);

create policy "Users can delete their own page selections"
